...
def spec():
    return KimiK3Spec(
        ...
        moe=LatentMoeSpec(
            896, 16, 3584, 3072, 2, True, "sigmoid", "noaux_tc", True, 1, 1, "1", "situ", "4", "25"
        ),
        rms_norm_epsilon="1/100000",
        activation_dtype="bfloat16",
        tied_embeddings=False,
        max_context=1048576,
        checkpoint_quantization="mxfp4-e2m1-group32-e8m0",
    )
...
def depth(graph, service=lambda operation: 1):
    ...
    for operation in graph.operations:
        finished[operation.operation_id] = max(
            (finished[name] for name in operation.depends_on), default=0
        ) + service(operation)
...
def test_exact_heterogeneous_geometry_and_storage_have_separate_names():
    ...
    assert [layer.index for layer in model.layers if layer.writes_residual_snapshot] == list(
        range(0, 85, 12)
    )
    state = model.retained_state(sequences=1, cached_tokens=1, current_tokens=1)
    assert state == {
        "kda_recurrent_bytes": 434110464,
        "kda_convolution_history_bytes": 15261696,
        "mla_cached_history_bytes": 27648,
        "mla_new_token_state_bytes": 27648,
        "residual_snapshot_capacity_bytes": 114688,
        "residual_current_stream_bytes": 14336,
    }
    ...
    routed = [
        w for w in weights if w.name in {"moe.expert_gate", "moe.expert_up", "moe.expert_down"}
    ]
    assert sum(w.parameters for w in routed) == 2722740830208
    shared = [
        w for w in weights if w.name in {"moe.shared_gate", "moe.shared_up", "moe.shared_down"}
    ]
...
@pytest.mark.parametrize(
    "prefill,visits,critical,slow_shared",
    [(True, 3244, 1812, 3744000), (False, 3268, 1860, 3792000)],
)
def test_complete_original_graph_and_shared_branch_join(prefill, visits, critical, slow_shared):
    ...
    assert (
        depth(
            ...
            lambda op: (
                10000
                if op.work.kernel.startswith("kimi_k3.moe.shared_")
                and op.work.kernel != "kimi_k3.moe.shared_routed_add"
                else 1000
            ),
        )
        == slow_shared
    ...
    assert {
        op.correlation.layer for op in graph.operations if op.correlation.layer is not None
    } == set(range(93))
    assert all(
        op.work.scope == "logical-operator"
        and op.work.hbm_bytes is None
    ...
    caches = [
        op for op in graph.operations if op.work.kernel == "kimi_k3.mla.compressed_cache_commit"
    ]
...
@pytest.mark.parametrize(
    "tokens,prior,pairs", [(1, 0, 1), (4, 0, 10), (16, 0, 136), (1, 16, 17), (1, 256, 257)]
)
def test_inclusive_causal_pairs_and_attention_algorithm_coefficients(tokens, prior, pairs):
    ...
    graph = lower(record)
    score = next(
        op.work for op in graph.operations if op.work.kernel == "kimi_k3.mla.query_key_product"
    )
    value = next(
        op.work
        for op in graph.operations
        if op.work.kernel == "kimi_k3.mla.probability_value_product"
    )
...
@pytest.mark.parametrize(
    "record",
    [step(tokens=2, prior=1), step(prefill=False, tokens=2), step(prefill=False, prior=1048576)],
)
...
def test_unbound_or_logical_demand_never_advances_runtime_even_with_duration():
    for work in (
        ComputeWork("logical", flops=None, hbm_bytes=None, scope="logical-operator"),
        ComputeWork(
            "logical", flops=0, hbm_bytes=0, nominal_duration_ps=1, scope="logical-operator"
        ),
    ):
        ...
        valid = replace(
            graph,
            operations=(
                replace(graph.operations[0], work=ComputeWork("valid", nominal_duration_ps=7)),
            ),
        )
...
def test_synthetic_binding_preserves_logical_topology_with_distinct_identity():
    ...
    assert (
        unbound_execution_graph_record(bound).record_id
        != unbound_execution_graph_record(original).record_id
    )
    ...
    for before, after in zip(original.operations, bound.operations, strict=True):
        assert (before.operation_id, before.logical_queue, before.depends_on) == (
            after.operation_id,
            after.logical_queue,
            after.depends_on,
        )
...
def test_logical_demand_rejects_goal_and_physical_bottleneck_consumers():
    ...
    graph = ExecutionGraph(
        ...
        0,
        0,
        (ExecutionOperation("op", 0, "queue", ComputeWork("one", nominal_duration_ps=7)),),
    )
    runtime = CoarseDeviceRuntime()
    runtime.execute(graph)
    for scope in ("logical-operator", "synthetic-operator"):
        changed = replace(
            graph,
            operations=(
                replace(graph.operations[0], work=replace(graph.operations[0].work, scope=scope)),
            ),
        )
        with pytest.raises(ValueError):
            classify_runtime(changed, runtime.last_report, {}, None)
        if scope == "logical-operator":
            with pytest.raises(ValueError, match="logical-operator"):
                render_serial_execution_graph_goal(changed)


@pytest.mark.parametrize("batch", [1, 3])
def test_history_grows_only_the_declared_mla_capacity(batch):
    states = [
        spec().retained_state(sequences=batch, cached_tokens=batch * history, current_tokens=batch)
        for history in (0, 16, 256)
    ]
    assert [s["kda_recurrent_bytes"] + s["kda_convolution_history_bytes"] for s in states] == [
        batch * 449372160
    ] * 3
    assert [s["mla_cached_history_bytes"] for s in states] == [
        batch * 27648 * history for history in (0, 16, 256)
    ]


def test_frozen_per_layer_partition_is_complete_and_source_specific():
    import json
    from pathlib import Path

    frozen = json.loads(
        (Path(__file__).parents[1] / "examples/kimi_k3_structure_v1/expectations.json").read_bytes()
    )
        ...
        for layer in range(93):
            ...
            expected_attention = (
                ...
                if attention == "kda"
                    ...
                    + partition["mla_expanded_extra" if prefill else "mla_absorbed_extra"]
                )
            )
            expected_ffn = partition["dense_ffn" if layer == 0 else "latent_moe"]
            assert sorted(
                name.split(".", 1)[1] for name in names if name.startswith(attention + ".")
            ) == sorted(expected_attention)
            assert sorted(
                name.split(".", 1)[1]
                for name in names
                if name.startswith("dense." if layer == 0 else "moe.")
            ) == sorted(expected_ffn)
            residual = [name for name in names if name.startswith("residual.")]
            assert len(residual) == len(partition["per_layer_common"]) == 3
            assert residual[0].startswith("residual.attention_sources_")
            assert residual[1].startswith("residual.mlp_sources_")
            assert residual[2] == "residual.ffn_delta_prefix_add"
        assert [
            op.work.kernel.removeprefix("kimi_k3.")
            for op in graph.operations
