...
def arguments(tmp_path, framework="vllm"):
    ...
    identity = FrameworkIdentity(
        ...
        declared.get("source_tree"),
        "unit-config-seam",
    )
    projection = FrameworkConfigurationProjection(
        identity,
        "unit-native-projection",
        declared["architecture_binding"],
        declared["text_implementation"],
        kimi_k3_stack=model.geometry,
    )
...
def test_inventory_preserves_unknown_demands_and_total_case_visits(inventory):
    ...
    for case in inventory.cases:
        assert sum(p.logical_launch_count for p in case.kernel_projections) == (
...
def test_old_uniform_inventory_cannot_claim_unknown_logical_demand():
    for path in sorted((ROOT / "offline/calibration/model-inventories").glob("*.json")):
        ...
        if "cases" in obj and "layer_types" not in obj["model"]["geometry"]:
            obj["cases"][0]["kernel_projections"][0].update(
                scope="logical-operator", aggregate_flops=None, aggregate_hbm_bytes=None
            )
            with pytest.raises(ValueError, match="scope disagrees"):
                ModelKernelInventory.from_obj(obj)
            break
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda s: s["graph_cells"][3].update(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda t: t["quantization_config"]["config_groups"]["group_0"]["weights"].update(
            group_size=64
        ),
        lambda t: t["quantization_config"]["config_groups"]["group_0"]["weights"].update(
            dynamic=True
...
def test_native_policy_changes_are_not_silently_normalized(mutation):
    ...
    layers = tuple(
        "mla" if i + 1 in text["linear_attn_config"]["full_attn_layers"] else "kda"
        for i in range(93)
    )
    assert (
        validate_kimi_k3_native_contract(
            SimpleNamespace(**text), layers, quant_method="compressed-tensors"
        )
        == "mxfp4-e2m1-group32-e8m0"
    )
    modified = copy.deepcopy(text)
    mutation(modified)
    with pytest.raises((TypeError, ValueError)):
        validate_kimi_k3_native_contract(
            SimpleNamespace(**modified), layers, quant_method="compressed-tensors"
        )
