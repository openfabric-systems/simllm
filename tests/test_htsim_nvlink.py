...
@pytest.mark.parametrize("filename,product,chips", (
    ("nvlink-domain-model.svg", "A100", 6),
    ("dgx-h100-nvlink.svg", "H100", 4),
))
def test_dgx_figures_preserve_vector_product_and_queue_evidence(filename, product, chips):
    root = ElementTree.parse(ROOT / "resources" / "figures" / filename).getroot()
    labels = ["".join(node.itertext()) for node in root.findall(f".//{SVG}text")]
    assert any(f"DGX {product}" in label and "one NVLink switching tier" in label
               for label in labels)
    assert {label for label in labels if label.startswith("NVSwitch ")} == {
        f"NVSwitch {i}" for i in range(1, chips + 1)
    }
    assert {label for label in labels if label.startswith("GPU ") and label[4:].isdigit()} == {
        f"GPU {i}" for i in range(8)
    }
    for boundary in ("GPU egress", "Switch input", "Crossbar + output link", "GPU ingress"):
        assert boundary in labels
    assert any("declared queue sizes" in label for label in labels)
    assert any("independent point-to-point links" in label for label in labels)
    assert root.findall(f".//{SVG}path")
    assert not root.findall(f".//{SVG}image")  # Export stays vector, with searchable text.
