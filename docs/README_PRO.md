...
| [backends](modules/backends.md) | Implemented: htsim invocation/parsing with per-layer, exact-sampling and GOAL-padding step-sink precision, plus native C++ RNIC SQ/CQ, network-port and shared PCIe transaction slices; the modular device entry point and versioned session run records with the policy-invariant hardware hash are landed; the virtual host-memory model with the QPC translation asymmetry and the three submission-source shapes with owned CQ consumers are landed; htsim composition remains open with its Tier A acceptance harness and wrapper design prepared | BACK-2/8-9/11-17/19-20; backend-repo HTSIM-1/2/4-9, ATLAHS-1 |
