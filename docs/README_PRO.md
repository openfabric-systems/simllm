...
**120 of 317 registered tasks closed.** 197 remain open across ten modules.
...
| [adapters-vllm](modules/adapters-vllm.md) | 12 | 30 | `███████░░░░░░░░░░░` |
...
| [pd_session_queue_onset_v1](../examples/pd_session_queue_onset_v1/RESULTS.md) | The VLLM-41 queue-onset study: a lower offered-load ladder frozen before running, the first queue-dominated segment predicted with quantitative bands derived from the imported measured surface and the arrival process alone (fitting from observed curves refused by the entry), and batching service reported separately from arrival-to-prefill and handoff-to-decode waits | VLLM-41 closed on a predictive success: the derived onset prediction of 225 to 230 requests per second (220 to 225 admitted by uncertainty) meets an observed onset of 210 to 220 in all six configurations, scheduler-wait bands held 30 of 30 while the batching-service bands missed 16 of 30 (registered as VLLM-42), conservation is exact at 4,992 admissions with zero TTFT residual, the validated 250-to-8,000 monotonic direction stands unrescored, and every preservation manifest is byte-identical |
