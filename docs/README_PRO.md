...
**80 of 193 registered tasks closed.** 113 remain open across ten modules.
...
| [adapters-sglang](modules/adapters-sglang.md) | Implemented: SimTpModelWorker via plugin entry point at pinned commit, live CPU-engine smoke, exact sample attribution, and the simulated vendored GroupCoordinator on the shared communicator base; the dispatch layer identity is now taken from SGLang itself rather than assumed by the adapter, as a component slice that does not yet reach the metric chain | [15 open](modules/adapters-sglang.md#open-tasks) |
