...
| NIC WQE fetch, payload DMA, CQE write | NIC | composed htsim session live: Tier B drives the chain into request TTFT/TPOT; packet-issue timestamps open | [HTSIM-9](modules/backends.md#open-tasks) | [rnic_live_v1 Tier B](../examples/rnic_live_v1/RESULTS.md), [rnic_pcie_v1](../examples/rnic_pcie_v1/RESULTS.md) |
...
| [rnic_live_v1 Tier B](../examples/rnic_live_v1/RESULTS.md) | Live reachability: composed native timing flows through ExecutionGraph, the DeviceRuntime, CompletionEvent and StepResult into the first network-affected TTFT and TPOT claim | All 8 single-WQE and 4 FIFO rows exact with six scored families at full genuine-risk fractions; D adds exactly 1,000 ps to live TTFT and decode TPOT, doubling the rate halves the wire term, FIFO W1 wait equals L through the live chain; four bypass profiles byte-identical to the protected reference; independently reproduced byte for byte from separately compiled binaries; the claim stays scoped to the frozen isolated fixture with packet-issue timestamps outside it |
...
- **M4 (in progress).** Closed loop validating M2/M3. Landed and
  ...
  against the composed binary with the first live TTFT/TPOT claim;
  BACK-8 closed; HTSIM-9 stays open for packet-issue evidence under
  BACK-25 and BACK-26; the frozen expectations live in
