...
**128 of 361 registered tasks closed.** 233 remain open across 11 modules.
...
| [deploy](modules/deploy.md) | The backend-free planning rung: strict deployment candidates, physical feasibility, stamped capacity estimates, role-pool rate matching, deterministic operating frontiers and promotion through the structural simulation contracts | [16 open](modules/deploy.md#open-tasks) |
...
| [traffic](modules/traffic.md) | Semantic collectives to physical flows: TP and MoE patterns with captured expert routing, calibrated collective floors and NVLink forms, GOAL rendering, live framework schedule producers, and the Slingshot-class fabric calibration | [49 open](modules/traffic.md#open-tasks) |
...
| [adapters-vllm](modules/adapters-vllm.md) | The no-fork vLLM adapter: `SimExecutor` on the pinned release, the flagged worker/runner skeleton, simulated communicators and placement export | [22 open](modules/adapters-vllm.md#open-tasks) |
...
| [matched_seam_frontier_v1](../examples/matched_seam_frontier_v1/RESULTS.md) | The corrected matched-seam deployment frontier: the imported measured operation surface carried through the deployment estimator with dependency traces proving SimLLM's roofline and fitted values cannot reach a scored root, all eight external empirical adjustments source-attributed, complete fresh-process determinism and packet-priced networking compared with an explicitly zero-service network arm | MIXED, non-void, after the first publication was honestly voided against its impossible FG-1 and retained in full: all eight corrected guards hold, S passes 13 of 13, R 10 of 10, F 12 of 13, M 2 of 2 under its narrowed packet-priced versus unpriced-network scope and W 1 of 1; F-2-09 remains a 0.607495 refutation, the five published rounding-boundary crossings and all eight remove-one sensitivity ranges are disclosed, DEPLOY-12 stays open for the LogGOPSim-priced third arm, DEPLOY-13 owns the rounded-axis residual and COMP-88 owns promotion of the study-local adjustment table |
...
| [nvlink_rnic_comparison_v2](../examples/nvlink_rnic_comparison_v2/RESULTS.md) | The round-two transport study the maintainer directed: the mapping audit of the merged comparison, rnic-nn-fluid as the true null reference, and the declared simulated incast mesh to degrees 4, 8 and 16 with tail and fairness metrics | The maintainer's diagnosis is CONFIRMED and quantified: the degree-3 long-flow tail was MAPPING_DEFICIT_IN_FAIR_SHARE_ENTITY_NOT_CAPACITY_VALUE (the receiver was granted the identical 207.101922 GB/s plateau in both studies, but overlapping application transfers were admitted as independent max-min flows), and the entity-ratio prediction 601 over 360 equals 1.669444 matches the observed 1.664553 to 0.294 percent; corrected, rnic-nn returns left of NVLink at 18.120617 against 18.145397 us. The fluid null is mechanically exact at 0 ps disagreement with the continuous-service oracle across all 42 cells, and the fluid-left-of-everything hypothesis is honestly refuted in 13 of 252 comparisons by an identified packetization effect at 256 B and 1 KiB. The higher-degree hypothesis is also REFUTED for equal-size flows: at degrees 4, 8 and 16 the two packet transports agree within 0.13 percent and the Jain fairness index is exactly 1.000000 for all three transports, so this workload cannot discriminate fair-share policy. Small-flow incast is disclosed as a model prediction with no hardware counterpart, since true-sync launch is not constructible through sequential PCIe writes |
...
| [core66 blocked capture](../examples/deployment_curve_v1/core66_capture_result.md) | The CORE-66 round: the registered EP72 cell confronted with the cluster's real inventory, the feasible EP12 cell frozen with its full deviation ledger, and a reusable guarded evidence reader committed | BLOCKED with zero GPU-hours consumed and zero physical bindings obtained: the registered 72-rank capture is impossible on a cluster with twelve GH200 GPUs, and the frozen feasible EP12 cell was not submitted because the worktree went protocol void first, on pre-reader listing and on two pre-commit full test runs that the integrator's own protocol had not exempted; the movement stays null with neither correction direction priced and both the 1 over 64 and 1 over 9 scales undecided, while the frozen cell, its deviation ledger and the reusable guarded reader carry forward as the specification for the corrected round |

| [local_shard_kernel_collector_v1](../examples/local_shard_kernel_collector_v1/RESULTS.md) | The COMP-50 local run boundary: one content-addressed request keeps logical parallelism separate from a physical shard and drives an external framework target with deterministic synthetic inputs; the candidate result must echo model, dispatch, shard, input and device ISA and close every relative sample blob by size and SHA-256 | PASS on the one nonphysical conformance evidence class, all 4 of 4 tensor-parallel and batch cells pass and repeat byte-identically; this makes the local run slice literal while COMP-50 stays open on the remaining doctor, compiler, package and submission surfaces, COMP-1 stays open on silicon evidence, and no GPU or network constant, TTFT, TPOT or framework coverage claim changes |
| [nvlink_credit_arbitration_v1](../examples/nvlink_credit_arbitration_v1/expectations.md) | The credit-structure documentation and arbitration study the maintainer's tail question opened: what NVLink credits actually are, why the model's default is what it is, the alternative policies made selectable, and three registered NV4 measurements that would decide the open internals | The physics is documented from public vendor and encyclopedic description (clearly not our measurement): credits are HARD-ALLOCATED PER LINK AND PER VIRTUAL CHANNEL, never shared across senders, so cross-sender credit capture is not an available mechanism and incast contention lives downstream at receiver ingress, memory acceptance or an NVSwitch crossbar output; the release-aware round robin default is therefore retained but now DOCUMENTED with that reasoning, with static interleave and greedy capture selectable and every existing study pinned byte-identically to its legacy policy. The simulated achieved-share table discriminates the policies sharply (at degree 16 greedy gives the elephant 99.760 GB/s while the small senders collapse to 6.858 through 7.482, against 12.938 each under round robin), and TRAF-73 registers the three deciding cells: the credit-window knee sweep, the per-sender-versus-shared pool sweep, and the sustained unequal-offered-rate arbitration test that measures fairness with long flows only |
| [nvlink_incast_validation_v1](../examples/nvlink_incast_validation_v1/RESULTS.md) | The maintainer-directed hardware-against-simulation validation at incast degrees 1, 2 and 3 on one NV4 node, long flows only, with the launch-skew negligibility assumption frozen as a fatal guard | VOID by its own guard, and the void is the finding: the measured degree-3 256 KiB launch-skew fraction reached 10.501 percent against the frozen 10.000 percent ceiling, refuting the assumption that the chosen rungs were long enough. The retained diagnostics show why the assumption failed so badly: hardware completions of 100 to 171 microseconds for 256 and 512 KiB transfers imply 2.2 to 3.5 GB/s per flow where the wire supports about 94, so these rungs measure launch and synchronization overhead rather than transfer, exactly the effect the PCIe serialization caveat predicts. All 42 observations, every inherited corrected-capture guard and the 59-artifact preservation guard passed; degrees 4, 8 and 16 remain declared simulation-only; closure needs a new freeze with far larger rungs or the sustained-rate methodology |
| [core66 capture attempts](../examples/deployment_curve_v1/core66_ep4_fallback_result.md) | Four frozen CORE-66 cells attempted against the real cluster: EP12, EP8, EP4 with DeepEP and EP4 with the fallback backend, each with its own deviation ledger and refusal or failure record preserved | Every cell blocked before profiling, with the blocker moving one layer inward each time and each one recorded: EP12 exceeded the 4-GPU-per-job policy, EP8 needed a gpu_hourly QoS absent from the association, EP4 with DeepEP allocated and validated the recovered CUDA 12.9 profiler environment but hit a DeepEP wheel built for CUDA 13, CPython 3.12 and x86-64 against a cu129, CPython 3.11, aarch64 host, and EP4 with the fallback backend allocated and then stopped on a missing orjson import in the same environment; total cost 0.15 GPU-hours with zero of 37 identities bound, zero HBM counters reached and the calibration movement null throughout. The finding is that no validated SGLang runtime exists for GH200 aarch64 (the earlier lane work validated vLLM only), so CORE-66's blocker is an environment deliverable rather than a capture one |
| [NVLink mechanism reverse engineering](../docs/design/nvlink-mechanism-reverse-engineering.md) | The maintainer-directed open-document reverse engineering of the NVLink mechanism, done before any number tuning: link layer, flow control, virtual channels, switch architecture and per-generation physical constants, every claim cited as PUBLIC_DOCUMENT, ending in the per-parameter reconciliation of the three-module model | Ten parameters CONFIRMED, six CONTRADICTED and twelve honestly UNDOCUMENTED, with the deciding correction that the documented flit is 16 bytes rather than 128, making the model's 272-byte unit one 17-flit packet occupancy rather than a universal packet or credit quantum; the contradicted set (optional address-extension and byte-enable flits, explicit virtual-channel and traffic-class identity, receiver-originated credit release, the missing CRC-acknowledge-replay path, and port-VOQ-grant switch arbitration) becomes the registered follow-on model alignment with expected signed effects stated, and nothing in the module changed in this task |
