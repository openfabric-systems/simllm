...
|---|---|---|
| [core](modules/core.md) | Implemented: virtual clock, step records, execution contracts, incremental append validation, the coarse DeviceRuntime, the completion reduction to StepResult and per-request TTFT/TPOT with seven-component critical-path attribution, and the structural RNIC network seam consuming composed native observations; the demonstrated CORE-15 live-seam clauses closed on Tier B, with the same-graph comparison retained as CORE-21 | CORE-3/8/9/10/11/12/13/14/17/21, BRIDGE-1 |
