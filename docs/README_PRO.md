...
**96 of 242 registered tasks closed.** 146 remain open across ten modules.
...
| [compute](modules/compute.md) | 6 | 28 | `████░░░░░░░░░░░░░░` |
| [placement](modules/placement.md) | 0 | 3 | `░░░░░░░░░░░░░░░░░░` |
| [traffic](modules/traffic.md) | 14 | 46 | `█████░░░░░░░░░░░░░` |
| [goal](modules/goal.md) | 1 | 1 | `██████████████████` |
| [preplay](modules/preplay.md) | 11 | 15 | `█████████████░░░░░` |
| [backends](modules/backends.md) | 28 | 57 | `█████████░░░░░░░░░` |
| [adapters-vllm](modules/adapters-vllm.md) | 7 | 23 | `█████░░░░░░░░░░░░░` |
| [adapters-sglang](modules/adapters-sglang.md) | 5 | 24 | `████░░░░░░░░░░░░░░` |
...
| [merlin_fabric_flow_capture_v1](../examples/merlin_fabric_flow_capture_v1/RESULTS.md) | The Slingshot calibration reference dataset: long-running NCCL per-chunk completion series over the Merlin Cassini fabric, with flows long-running so the measured CPU tracer floor stays under one percent of chunk service, across a sustained solo stream, an incast ladder, step-wise flow joins and a mixed A100-plus-GH200 pair | PARTIAL by queue, clean by evidence: 10 of 18 frozen relations evaluated and all 10 pass, every fatal guard held over 390,228 verified chunks, and 8 relations unevaluated because cluster reservations pin the 4-and-5-node allocations until after publication, reported queued with reproduction paths; the dataset is byte-locked (78 tracked files and their guard evidence under one CI-enforced manifest hash) and no fabric-model claim is made; headline observations: a degree-2 incast raises aggregate goodput 1.71 times because each flow rides its own source stack and destination port, a staggered join on pre-established connections costs the established flow nothing (post-specified cell) while simultaneous starts take 119 seconds to settle, burst and sustained rates differ by tens of percent with pair-dependent sign so neither anchors the other, and the first cross-architecture NCCL communicator here (x86_64 plus aarch64 over one fabric subnet) shows a 2.77x direction asymmetry with the Grace-sourced leg slow; registers TRAF-51 (the htsim Slingshot comparison) and TRAF-52 (the queued families and the freeze-2 GH-to-GH arm) |
| [merlin_ss_fabric_calibration_v1](../examples/merlin_ss_fabric_calibration_v1/RESULTS.md) | The TRAF-51 Slingshot calibration comparison: the hosted htsim ss-dragonfly fabric against the byte-locked Merlin capture dataset through a frozen composition rule that separates the measured endpoint host-stack floor from fabric serialization, on a declared single-switch Merlin instance whose every parameter carries provenance | PARTIAL as pre-declared, clean by evidence: no fatal guard fired, all 11 simulation rows pass with three hand-derived exact oracles confirmed to the bin (first-chunk 340.417 us, exact serialization doubling at half rate, exact framing shift to 3 parts per million), and all 10 conditional composed rows confirm with every captured steady quantity reproduced within 7.3 percent from solo anchors alone; the endpoint floor is separated at 78.7 to 84.5 percent of chunk life, so the fabric is calibrated for steady-state solo, incast and staggered-join behavior at the captured distinct-port mappings and loads, while the 119-second simultaneous-start transient is registered as un-modeled endpoint dynamics rather than fitted, the mixed pair stays excluded by the frozen reason, and the open-loop shared-egress artifact regime is echoed only as a positive control; registers TRAF-53 and HTSIM-29 to HTSIM-31, TRAF-51 stays open narrowed, and the rnic-ss endpoint claim does not move |
...
- **M5 (in progress).** All-to-all traffic studies (MoE expert
  ...
  The Slingshot fabric is hosted in the htsim backend and calibrated
  for steady-state flow families against the Merlin captures
  (TRAF-51 partial, examples/merlin_ss_fabric_calibration_v1;
  `rnic-ss` exercise from simllm remains HTSIM-1).
