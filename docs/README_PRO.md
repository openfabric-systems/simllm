...
| NIC WQE fetch, payload DMA, CQE write | NIC | PCIe transaction slice standalone; the modular device entry point is landed, htsim binding open | [BACK-8](modules/backends.md#open-tasks), [HTSIM-9](modules/backends.md#open-tasks) | [rnic_device_v1](../examples/rnic_device_v1/RESULTS.md), [rnic_pcie_v1](../examples/rnic_pcie_v1/RESULTS.md) |
...
|---|---|---|
| [core](modules/core.md) | Implemented: virtual clock, step records, execution contracts, incremental append validation, the coarse DeviceRuntime, and the completion reduction to StepResult and per-request TTFT/TPOT with seven-component critical-path attribution; structural completion stays conditional on CORE-15; BRIDGE-1 closed for the pinned-binary prepared-replay scope with the online stateful session moved to BRIDGE-2, CORE-24 and HTSIM-18 | CORE-3/8/9/10/11/12/13/14/15/17/24, BRIDGE-2/3 |
...
- **M4 (in progress).** Closed loop validating M2/M3. Landed and
  ...
  the composed native RNIC and htsim session (Tier A PASSED against the composed binary; BACK-8 and HTSIM-9 stay open for Tier B;
