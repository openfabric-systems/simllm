...
window) are merged. The paired SimLLM pin is now backend commit
`f8e1ee923a9c108cd698786c1824b9722d22d0e1`, which adds the persistent flow
session. Until that commit is reachable from backend main, its carrying
branch is append-only under the backend repository policy. The same HTSIM
sources build on Linux with GCC/Clang and on Windows with MSVC. From the
SimLLM root, the supported helper commands are:
...
landed under BACK-25 and BACK-26 the same day, below. HTSIM-9 remains open for
a composed run showing first-packet and last-packet issue on those interfaces.
ABI-v1 network acceptance and whole-flow terminal events are not substitutes
for packet issue.
