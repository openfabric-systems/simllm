...
- BACK-70 (Completeness; P2; S): make the collective floor identity tests
  honest about the pin. `tests/test_collective_floor_study.py` compares the
  environment's `htsim_rnic` and `txt2bin` with the sha256 the
  collective_floor records name, which belong to a build of an htsim commit
  older than the current pin; under every current local environment they
  fail (803a0188 against cfb5014a, c9512f7d against f3745f34) and only skip
  on CI where the variables are unset, so every wave landing of 2026-09-07
  carried two expected failures. Acceptance: either the records are
  refreshed under the current pin with the family B and mixed D8 artifacts
  regenerated byte for byte, or the identity test skips with a named reason
  when the environment's binary belongs to a different pin than the record,
  and a regression test pins the chosen behavior.
