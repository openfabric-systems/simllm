...
| BACK-50 | The effective-hardware snapshot builds its `dma` and `host_memory` objects from key sets that predate endpoint identities, so two devices differing only in fabric endpoint identity share one `hardware_config_sha256`. |
