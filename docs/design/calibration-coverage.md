...
| DeepSeek-V3 | `deepseek-ai/DeepSeek-V3` class checkpoint (open-weight 671B MoE, 37B active, multi-head latent attention, shared plus routed experts, FP8-native); exact revision and hashes pinned at its extraction freeze | Planned as the CORE-54 flagship column: COMP-67 owns the MLA and DeepSeek MoE families plus the deployment-sharded projection; the public deployment anchors live in [the disclosures dossier](../papers/deepseek-deployment-disclosures.md) |
