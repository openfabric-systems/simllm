# TRAF-73 NVLink identification worker sizing

## Aligned freeze, capture and classification

- Date: 2026-09-01.
- As-of commit: `4073b4e762d2a209e8f9f642e360054290d41465`.
- Scope: re-derive the H1, H2 and H3 candidate predictions through the
  mechanism-aligned NVLink authority, freeze their hardware discriminators,
  execute the three serially paced NV4 hardware families, score every
...
- Exclusions: no edit under `simllm/`, no candidate-profile or module-default
  ...
  and no work in deploy, serving-band, collective, MiniMax, step-sink,
  collective-floor or locality surfaces.
- Owner: TRAF-73 worker on `codex/traf73_credit_identification`.
- Dependencies: the merged TRAF-80 aligned authority, the retained TRAF-70
  producer lineage and fatal-guard observables, the historical TRAF-73
  simulation publication, one free serially paced Merlin allocation, and the
  integrator-assigned TRAF-85 identifier remaining unused on main if a
  promotion residual is required.
- First reviewable slice: an expectations-only commit containing aligned
  per-policy predictions, exact hardware outcome selectors, physical floors
  and ceilings, H1 no-break interpretation, H2 aggregate-outstanding-byte
  discriminator, H3 steady-window rules, immutable input identities and
  focused freeze checks. It precedes every new hardware observation.

### Expected files

- Create an aligned expectations generator plus JSON and Markdown authority
  under `examples/nvlink_credit_arbitration_v1/`, without mutating the
...
- Create a resumable study-side campaign runner, a serially paced Merlin batch
  ...
  source and observables rather than creating another hardware producer.
- Create compact result JSON, Markdown and discriminator tables under the same
  study directory after the non-void run.
- Modify `docs/modules/traffic.md` only after scoring. Remove TRAF-73 only if
  all three frozen questions are classified; otherwise retain it. Register
  only a model-value promotion residual as TRAF-85, with the exact deciding
