...
def make_plan(config):
    ...
    for width in config["widths"]:
        for repeat in range(config["auto_repeats"]):
            auto.append(dict(lane="timing", arm="auto", width=width,
                             repeat=repeat, grid="dense_grid", env={}, graph=0))
        arms = []
        for algo in config["algorithms"]:
            for proto in config["protocols"]:
                arms.append((algo + "_" + proto, {"NCCL_ALGO": algo,
                             "NCCL_PROTO": proto}, 0))
        for count in config["fixed_channels"]:
            arms.append(("Ring_LL128_cta%d" % count,
                         {"NCCL_ALGO": "Ring", "NCCL_PROTO": "LL128",
                          "NCCL_MIN_CTAS": str(count), "NCCL_MAX_CTAS": str(count)}, 0))
        arms.append(("auto_graph", {}, config["graph_replays"]))
        for arm, env, graph in arms:
            for repeat in range(config["control_repeats"]):
                controls.append(dict(lane="timing", arm=arm, width=width,
                                     repeat=repeat, grid="control_grid", env=env, graph=graph))
            diagnostics.append(dict(lane="diagnostic", arm=arm, width=width,
                                    repeat=0, grid="control_grid", env=env, graph=graph))
        diagnostics.append(dict(lane="diagnostic", arm="auto", width=width,
                                repeat=0, grid="dense_grid", env={}, graph=0))
    legacy = [dict(lane="legacy", arm="auto", width=0, repeat=r,
    ...
    return ([legacy[0]] + [r for r in auto if r["repeat"] == 0] + middle
            + [r for r in auto if r["repeat"] == config["auto_repeats"] - 1]
            + [legacy[-1]] + diagnostics)
...
def command_for(run, config, binary, legacy, output):
    ...
    return [str(binary), "-t", str(run["width"]), "-g", "1",
            "-b", str(grid["start_bytes"]), "-e", str(grid["end_bytes"]),
            "-i", str(grid["step_bytes"]), "-d", "float", "-o", "sum",
            "-w", str(config["diagnostic_warmup" if diagnostic else "timing_warmup"]),
            "-n", str(config["diagnostic_iterations" if diagnostic else "timing_iterations"]),
            "-N", "1", "-m", "1", "-a", "3", "-z", "0", "-c", "1",
            "-G", str(run["graph"]), "-U", "1" if diagnostic else "0",
            "-J", str(output)]
...
def main():
    ...
    base_env = {k: v for k, v in os.environ.items()
                if not k.startswith("NCCL_")}
    base_env["NCCL_DEBUG"] = "WARN"
    manifest = dict(schema="simllm-nccl-transition-campaign-v1",
                    architecture=args.architecture, freeze=args.freeze,
                    implementation=args.implementation,
                    expectations_sha256=hashlib.sha256(args.expectations.read_bytes()).hexdigest(),
                    inherited_nccl_environment={k: v for k, v in os.environ.items()
                                                if k.startswith("NCCL_")},
                    runs=[])
    path = args.output / "campaign.json"
    plan = make_plan(config)
    for sequence, run in enumerate(plan):
        stem = "%03d_%s_%s_w%d_r%d" % (
            sequence, run["lane"], run["arm"], run["width"], run["repeat"])
        ...
        record = dict(run, sequence=sequence, command=command, output=output.name,
                      log=log.name, started_unix=time.time())
        print("[%d/%d] %s" % (sequence + 1, len(plan), stem), flush=True)
        code, elapsed = execute(command, env, log, config["process_timeout_seconds"])
        record.update(returncode=code, elapsed_seconds=elapsed,
    ...
    path.write_text(json.dumps(manifest, indent=2) + "\n")
    print("Campaign complete: %d process records" % len(plan), flush=True)
