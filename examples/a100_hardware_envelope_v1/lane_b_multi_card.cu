// Lane B of the A100 hardware envelope study: four cards inside one node.
//
...
int main(int argc, char** argv) {
  ...
  // Peer access for every ordered pair.
    ...
    for (int b = 0; b < device_count; ++b) {
      ...
      if (can) {
        ...
        if (err != cudaSuccess && err != cudaErrorPeerAccessAlreadyEnabled) {
          std::fprintf(stderr, "FATAL peer enable %d->%d: %s\n", a, b,
                       cudaGetErrorString(err));
          return 23;
        }
      } else {
        std::fprintf(stderr, "FATAL no peer access %d->%d\n", a, b);
        return 24;
      }
    }
  }
