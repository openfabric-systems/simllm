...
checked-out commit at run time. That stamp does not meet the amendment's
literal wording, which promised the corrected commit; the tree that ran is
the corrected one, and the follow-up (`ce6f098a`) that binds each `-R`
heading index to the `Minor Number` of its `-q` record, so a swap of the
`Bus Id` lines together with the `-R` blocks is refused too, regenerated the
results again.

Two limits are disclosed rather than closed. Without the inventory table
the join's only authority for switch names is the dump, so a dump switch row
renamed to a bus id that exists nowhere else cannot be detected from the
dump alone; with the table it is refused. A links-only swap between two
GPUs under unchanged block headings and unchanged `-q` records is not
detectable from these sources either; it would need the switch side's own
port-to-GPU record, which this container does not expose.
