...
def graph_depth(graph, *, slow_shared=False):
    ...
    for operation in graph.operations:
        shared = (operation.work.kernel.startswith("kimi_k3.moe.shared_")
...
class Evidence:
    ...
    def check(self, name, actual, expected, *, kind="guard", family=None):
        ...
        if family is not None:
            row["family"] = family
        getattr(self, {"guard": "guards", "oracle": "oracles", "relation": "relations"}[kind]).append(row)
...
def environment(framework):
    env = dict(os.environ)
    env.update(PYTHONPATH=str(ROOT) + (os.pathsep + env["PYTHONPATH"] if env.get("PYTHONPATH") else ""),
...
def native_run(python, framework, code, args, output, name, *, overrides=None, reject=False):
    ...
    process = subprocess.run([str(python), "-c", code, *map(str, args)], cwd=ROOT,
    ...
    if reject:
        return process.returncode != 0 and b"state" in process.stderr
...
def extract(python, framework, checkpoint, output, name, suite_id=SUITE_ID):
    ...
    result = native_run(python, framework, code, [
        "extract", "--framework", framework, "--suite", suite_id,
        "--suite-root", ROOT / "offline/calibration", "--checkpoint-root", checkpoint,
        "--step-records", directory / "steps.jsonl", "--output-root", directory / "objects",
    ], output, name)
...
def storage_oracles(spec, frozen, evidence):
    ...
    values = {
        ...
        "shared_bf16_bytes_all_layers": 2 * total({"moe.shared_gate", "moe.shared_up", "moe.shared_down"}),
        ...
        "kda_total_persistent_bytes_per_sequence": state["kda_recurrent_bytes"] + state["kda_convolution_history_bytes"],
...
def check_graphs(inventory, suite, frozen, output, evidence):
    ...
    for case, record in zip(inventory.cases, case_records_from_suite(suite), strict=True):
        ...
        with gzip.GzipFile(filename=str(output / f"{key.replace(':', '-')}.graph.json.gz"), mode="wb", mtime=0) as stream:
            stream.write(raw)
        evidence.check(key + ":instance-join", unbound_execution_graph_record(graph).record_id, case.instance_graph_sha256)
        evidence.check(key + ":template-join", execution_graph_template_record(graph).record_id, case.template_graph_sha256)
        evidence.check(key + ":original-layers", sorted({op.correlation.layer for op in graph.operations if op.correlation.layer is not None}), list(range(93)))
        evidence.check(key + ":unbound-demand", all(op.work.hbm_bytes is None and op.work.scope == "logical-operator" and op.work.nominal_duration_ps is None for op in graph.operations), True)
        counts = {}
        for operation in graph.operations:
            family = operator_family(operation)
            counts[family] = counts.get(family, 0) + 1
        evidence.check(key + ":family-join", counts, {p.family_id: p.logical_launch_count for p in case.kernel_projections if p.logical_launch_count})
        evidence.check(key + ":visits", len(graph.operations), frozen["exact_oracles"][case.phase + "_logical_visits"], kind="oracle")
        evidence.check(key + ":depth", graph_depth(graph) // 1000, frozen["exact_oracles"][case.phase + "_dependency_depth"], kind="oracle")
        evidence.check(key + ":residual-sources", sum(dict(op.work.config).get("source_vectors", 0) for op in graph.operations), frozen["exact_oracles"]["residual_source_evaluations_per_current_token"], kind="oracle")
        caches = {op.operation_id for op in graph.operations if op.work.kernel == "kimi_k3.mla.compressed_cache_commit"}
        evidence.check(key + ":cache-frontier", len(caches & set(graph.completion_operation_ids)), 24)
        scores = [op for op in graph.operations if op.work.kernel == "kimi_k3.mla.query_key_product"]
        evidence.check(key + ":cache-causality", all(bool(caches & set(op.depends_on)) == (case.phase == "decode") for op in scores), True)
        score_flops = sum(op.work.flops for op in graph.operations if op.work.kernel in {"kimi_k3.mla.query_key_product", "kimi_k3.mla.probability_value_product"})
        ...
        family = "cold-prefill-inclusive-pairs" if case.phase == "prefill" else "decode-history-growth"
        evidence.check(key + ":attention-work", score_flops, expected_pairs * 24 * (61440 if case.phase == "prefill" else 208896), kind="relation", family=family)
        final_norm = next(op for op in graph.operations if op.work.kernel == "kimi_k3.final_norm")
        evidence.check(key + ":final-norm-axis", dict(final_norm.work.config)["tokens"], len(record.scheduled) if framework == "vllm" else shape.new_tokens)
        rows[case.case_id] = {"graph_sha256": digest(raw), "known_flops": sum(op.work.flops or 0 for op in graph.operations),
                                  "state": spec.retained_state(sequences=shape.sequences, cached_tokens=shape.cached_tokens, current_tokens=shape.new_tokens),
                                  "slow_shared_ps": graph_depth(graph, slow_shared=True)}
    for case_id, row in rows.items():
        if "-b3-" in case_id:
            baseline = rows[case_id.replace("-b3-", "-b1-")]
            evidence.check(framework + ":batch-work:" + case_id, row["known_flops"], 3 * baseline["known_flops"], kind="relation", family="batch-linear-work-and-capacity")
            evidence.check(framework + ":batch-state:" + case_id, row["state"], {name: 3 * value for name, value in baseline["state"].items()}, kind="relation", family="batch-linear-work-and-capacity")
    for phase, case_id in (("prefill", "prefill-b1-n1"), ("decode", "decode-b1-c1")):
        evidence.check(framework + ":frozen-shared-join:" + phase, rows[case_id]["slow_shared_ps"],
...
def request_loop(spec, framework, output, evidence):
    ...
    for prompt in (1, 4):
        for service in (1000, 2000):
            ...
            for index in range(3):
                ...
                record = StepRecord(index, clock.now_ps, scheduled=[ScheduledRequest(
                    "request", RequestPhase.PREFILL if index == 0 else RequestPhase.DECODE,
                    tokens, context_length=prompt + index)], num_sampled=1)
                ...
                with gzip.GzipFile(filename=str(output / f"{name}.events.json.gz"), mode="wb", mtime=0) as stream:
                    stream.write(canonical_bytes(execution_result_to_json(result)))
                expected = (3244 if index == 0 else 3268) * tokens * service
                evidence.check(name + ":serial-completion", reduced.step_latency_ps, expected, kind="oracle")
                evidence.check(name + ":event-inventory", len({event.operation_id for event in result.events}), len(graph.operations))
                evidence.check(name + ":synthetic-identity", bound.execution_id != graph.execution_id, True)
                metric = reduced.request_metrics[0]
                evidence.check(name + ":ttft", metric.ttft_ps, 3244 * prompt * service, kind="oracle")
                evidence.check(name + ":tpot", None if metric.tpot_ps is None else int(metric.tpot_ps), None if index == 0 else 3268 * service, kind="oracle")
                results.append(reduced.step_latency_ps)
            row = {"framework": framework, "prompt_tokens": prompt, "service_ps_per_new_token": service,
                       "ttft_ps": results[0], "tpot_ps": results[1], "jct_ps": clock.now_ps,
    ...
    for field in ("ttft_ps", "tpot_ps", "jct_ps"):
        for a, b in ((rows[0], rows[1]), (rows[2], rows[3])):
            evidence.check(f"{framework}:double-service:p{a['prompt_tokens']}:{field}", b[field], 2 * a[field], kind="relation", family="synthetic-request-service-scaling")
    evidence.check(framework + ":prompt-ttft-scaling", rows[2]["ttft_ps"], 4 * rows[0]["ttft_ps"], kind="relation", family="synthetic-request-service-scaling")
    evidence.check(framework + ":decode-prompt-independence", rows[2]["tpot_ps"], rows[0]["tpot_ps"], kind="relation", family="synthetic-request-service-scaling")
    return rows


def compatibility(arguments, frozen, evidence, output):
    baseline = frozen["compatibility"]["baseline_commit"]
    paths = subprocess.check_output(["git", "ls-tree", "-r", "--name-only", baseline], cwd=ROOT).decode().splitlines()
    selected = [p for p in paths if p.startswith("offline/calibration/model-inventories/") and p.endswith(".json")
                or p.startswith("offline/calibration/suites/") and p.endswith("/suite.json")]
    ...
    rows = []
    accepted = [ModelKernelInventory.from_obj(json.loads((ROOT / path).read_bytes())) for path in selected if path.startswith("offline/calibration/model-inventories/")]
    for suite_id, checkpoint in checkpoints.items():
        for framework in ("vllm", "sglang"):
            inventory, steps = extract(getattr(arguments, framework + "_python"), framework, Path(checkpoint), output,
                                       f"legacy-{suite_id}-{framework}", suite_id)
            matches = [item for item in accepted if item.suite == inventory.suite and item.framework == inventory.framework]
            evidence.check(f"native-legacy:{suite_id}:{framework}", inventory.record.record_id in {item.record.record_id for item in matches}, True)
            rows.append({"suite": suite_id, "framework": framework, "inventory_sha256": inventory.record.record_id, "steps_sha256": steps})
...
def main(argv=None):
    ...
    evidence = Evidence()
    summary = {"schema": "simllm-kimi-k3-structure-result-v1", "freeze_commit": FREEZE,
                   "source_commit": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT).decode().strip(),
                   "hardware_measurements": 0, "inventories": {}, "graph_cells": {}, "request_cells": []}
    ...
    try:
        ...
        for name in ("expectations.json", "expectations.md"):
            blob = subprocess.check_output(["git", "show", f"{FREEZE}:examples/kimi_k3_structure_v1/{name}"], cwd=ROOT)
        ...
        for name, expected in frozen["source_files"].items():
            evidence.check("source:" + name, digest((args.source_root / name).read_bytes()), expected)
        ...
        for framework in ("vllm", "sglang"):
            ...
            projection = inspect(python, framework, args.checkpoint_root, args.output_root, framework + "-inspect")
            projections[framework] = projection
            evidence.check(framework + ":native-geometry", projection["kimi_k3_stack"], spec.to_obj())
            repeats = []
            for repetition in range(2):
                inventory, step_sha = extract(python, framework, args.checkpoint_root, args.output_root, f"{framework}-repeat-{repetition}")
                repeats.append((inventory.record.record_id, step_sha))
            evidence.check(framework + ":repeat-identity", list(repeats[1]), list(repeats[0]))
            summary["inventories"][framework] = {"record_sha256": repeats[0][0], "steps_sha256": repeats[0][1]}
            summary["graph_cells"][framework] = check_graphs(inventory, suite, frozen, args.output_root, evidence)
            summary["request_cells"].extend(request_loop(spec, framework, args.output_root, evidence))
        for variable in ("SGLANG_MAMBA_SSM_DTYPE", "SGLANG_MAMBA_CONV_DTYPE"):
            evidence.check("native-state-override:" + variable,
                           inspect(args.sglang_python, "sglang", args.checkpoint_root, args.output_root,
                                   "reject-" + variable.lower(), overrides={variable: "float16"}, reject=True), True)
    ...
    summary.update(verdict="PASS" if evidence.valid else "VOID", behavioral_score=(
        {"families": sorted({row["family"] for row in evidence.relations}), "instances": len(evidence.relations)}
        if evidence.valid else None), fatal_findings=[row for row in evidence.guards + evidence.oracles + evidence.relations if not row["passed"]])
    ...
    summary["evidence_sha256"] = {name: canonical_sha256(rows) for name, rows in
                                  (("guards", evidence.guards), ("oracles", evidence.oracles), ("relations", evidence.relations))}
    write(args.output_root / "summary.json", summary)
    print(json.dumps({"verdict": summary["verdict"], "behavioral_score": summary["behavioral_score"], "fatal_findings": summary["fatal_findings"]}), flush=True)
