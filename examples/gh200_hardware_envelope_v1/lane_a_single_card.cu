// Lane A of the GH200 hardware envelope study: one card.
//
// Measures HBM bandwidth, dense BF16 GEMM throughput across the roofline
// crossover, kernel launch cost and the host link. Every timed block is
...
//
// This is the A100 lane A of commit 8509a2c with its architecture dependence
// removed: the HBM and tensor-core peaks are derived from device properties
// plus one table of BF16 dense FLOP per SM per cycle, the launch grid scales
// with the SM count, and both derived peaks are emitted so the scorer reads
// them rather than carrying a constant. On an A100 the derivation reproduces
// the 2,039.04 GB/s and 311.87 TFLOP/s the A100 study hardcoded.
//
...
// Eight blocks per SM at 256 threads is one full occupancy wave on both
// Ampere and Hopper, which cap resident threads per SM at 2048.
int grid_blocks(const cudaDeviceProp& prop) { return prop.multiProcessorCount * 8; }

// BF16 dense tensor-core throughput per SM per clock. Ampere issues 2048 and
// Hopper doubles it. Refusing an unknown architecture is deliberate: a wrong
// constant here silently rescales every efficiency in the report.
double bf16_flop_per_sm_cycle(int major, int minor) {
  if (major == 8 && (minor == 0 || minor == 6 || minor == 9)) return 2048.0;
  if (major == 9) return 4096.0;
  return 0.0;
}

// Nameplate HBM bandwidth from the reported memory clock and bus width. The
// reported clock is half the data rate, hence the factor two.
double hbm_peak_bytes_per_second(const cudaDeviceProp& prop) {
  return 2.0 * prop.memoryClockRate * 1e3 * (prop.memoryBusWidth / 8.0);
}
...
int main(int argc, char** argv) {
  ...
  const int kGridBlocks = grid_blocks(prop);
  const double flop_per_sm_cycle = bf16_flop_per_sm_cycle(prop.major, prop.minor);
  if (flop_per_sm_cycle == 0.0) {
    std::fprintf(stderr, "FATAL unknown BF16 tensor rate for compute capability %d.%d\n",
                 prop.major, prop.minor);
    return 23;
  }
  const double hbm_peak_bps = hbm_peak_bytes_per_second(prop);
  const double bf16_peak_flops =
      prop.multiProcessorCount * (prop.clockRate * 1e3) * flop_per_sm_cycle;
  ...
  j.num("bf16_flop_per_sm_cycle", flop_per_sm_cycle);
  j.num("hbm_peak_bytes_per_second", hbm_peak_bps);
  j.num("bf16_peak_flops", bf16_peak_flops);
  ...
  for (const GemmPoint& p : gemm_points) {
    ...
    j.num("t_mem_ms", bytes / hbm_peak_bps * 1e3);
    j.num("t_flop_ms", flops / bf16_peak_flops * 1e3);
    emit_clocks(j, "clocks_before", before);
    emit_clocks(j, "clocks_after", after);
    j.close_obj();
  }
