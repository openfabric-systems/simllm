#!/bin/sh
# GPU-free compile check for the TRAF-65 producer harness.
set -eu

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
DEVELOPMENT_ROOT=$(CDPATH= cd -- "${SCRIPT_DIR}/../../../.." && pwd)
TASK_ROOT=${TRAF65_LOCAL_ROOT:-${DEVELOPMENT_ROOT}/wave-runs/traf65}
...
"${CXX:-c++}" -x c++ -std=c++17 -O2 -Wall -Wextra -Wpedantic \
  ...
  -o "${BINARY}"
sha256sum "${SCRIPT_DIR}/nvlink_packet_lane.cu" "${BINARY}"
