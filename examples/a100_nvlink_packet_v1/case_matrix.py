"""Deterministic parameter points for the frozen TRAF-65 case catalog."""
...
def points_for_case(case: dict[str, Any]) -> tuple[SweepPoint, ...]:
    ...
    if ordinal == 8:
        return tuple(
            replace(base, point_id=f"{name}:mask={mask}", lane_mask=mask)
    ...
    if ordinal == 10:
        return tuple(
            replace(
                ...
                message_count=max(1, (256 << 20) // payload),
            )
    ...
    if ordinal == 18:
        return tuple(
            replace(base, point_id=f"{name}:pair=0->{destination}", destination=destination)
        ...
        for index in range(32):
            ...
            points.append(
                replace(
                    base,
                    point_id=f"{name}:blind={index:02d}",
                    payload_bytes=rng.choice((16, 64, 256, 4096, 1 << 20)),
                    source=source,
                    destination=destination,
                    sources=str(source),
                    destinations=str(destination),
...
def _message_count(payload_bytes: int) -> int:
    return max(32, min(4096, (64 << 20) // payload_bytes))
