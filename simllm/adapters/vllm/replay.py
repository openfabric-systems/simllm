...
class ReplayTokenSource:
    ...
    def __init__(self, run: PreplayReplayRun) -> None:
    ...
    @classmethod
    def from_path(cls, path: str | Path) -> ReplayTokenSource:
        """Load a strict replay-run record and verify its named trace."""

        return cls(read_preplay_replay_run(path))
    ...
    def _reported_output_indices(scheduler_output: Any) -> dict[str, int]:
        ...
        for request_id, output_count in zip(request_ids, output_counts, strict=True):
            index = int(output_count)
            if index < 0:
                raise RuntimeError(
                    f"replay request {request_id!r} has negative output index {index}"
                )
