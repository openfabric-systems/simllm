#ifndef SIMLLM_PORTS_PACKET_PORT_H
#define SIMLLM_PORTS_PACKET_PORT_H

#include <cstdint>

// Canonical C++ type identities retain their historical namespace so compiled
// wire clients keep the same ABI. New producers use the neutral aliases below.
// The records contain no queue ownership or RNIC lifecycle implementation.
...
enum class NetworkEventKind {
    // Transport retirement, which may follow destination visibility.
    Delivered,
    Dropped,
    // The session declares producer issue versus physical link grant.
    PacketTxStarted,
    PacketTxFinished,
    PacketRxArrived,
    EcnMarked,
...
struct NetworkTxDescriptor {
    ...
    // grant ownership of the corresponding producer or application objects.
    // Legacy wqe_id and qpn names are opaque for a peer producer; no RNIC
    // queue is created or implied by populating these correlation fields.
    WqeId wqe_id{0};
...
namespace simllm::ports {

using Picoseconds = rnic::Picoseconds;
using ProducerId = rnic::WqeId;
using FlowId = rnic::FlowId;
using PolicyContextToken = rnic::PolicyContextToken;
using PortToken = rnic::NetworkToken;
using PortSubmitStatus = rnic::NetworkSubmitStatus;
using PortEventKind = rnic::NetworkEventKind;
using PortEventScope = rnic::NetworkEventScope;
using PacketKind = rnic::NetworkPacketKind;
using DropEvidenceProvenance = rnic::DropEvidenceProvenance;
using PortLinkState = rnic::NetworkLinkState;
using DropLocation = rnic::DropLocation;
using DropReason = rnic::DropReason;
using PortTxDescriptor = rnic::NetworkTxDescriptor;
using PortCapabilities = rnic::NetworkPortCapabilities;
using PortSubmitResult = rnic::NetworkSubmitResult;
using PortEvent = rnic::NetworkEvent;
using PacketPort = rnic::NetworkPort;

inline constexpr std::uint32_t kPacketPortAbiVersionV1 =
    rnic::kNetworkPortAbiVersionV1;
inline constexpr std::uint32_t kPacketPortAbiVersionV2 =
    rnic::kNetworkPortAbiVersionV2;
inline constexpr std::uint32_t kPacketPortAbiVersion =
    rnic::kNetworkPortAbiVersion;

}  // namespace simllm::ports

#endif  // SIMLLM_PORTS_PACKET_PORT_H
