...
class _GraphBuilder:
    ...
    def kda(self, parent: str, layer: int) -> str:
        ...
        projected = {
            name: self.matrix(f"kda.{name}_projection", parent, h, width, layer=layer)
            for name in ("q", "k", "v")
        }
        convolved = {
            name: self.region(
                f"kda.{name}_convolution_silu",
                (projected[name],),
                layer=layer,
                channels=width,
                history=spec.convolution_width - 1,
        ...
        forget_b = self.matrix(
            "kda.forget_b_projection", forget_a, spec.head_dim, width, layer=layer
        )
        forget = self.region(
            "kda.forget_gate",
            (forget_b,),
        ...
        update = self.region(
            "kda.state_update_and_query",
            (normalized["q"], normalized["k"], convolved["v"], forget, beta_gate),
            layer=layer,
            heads=spec.heads,
            head_dim=spec.head_dim,
            sequences=self.shape.sequences,
            recurrent_dtype=spec.recurrent_dtype,
    ...
    def mla(self, parent: str, layer: int) -> str:
        ...
        q_b = self.matrix(
            ...
            q_norm,
            spec.query_rank,
            spec.heads * spec.expanded_score_dim,
            layer=layer,
        )
        ...
        cache = self.region(
            "mla.compressed_cache_commit",
            (kv_a, kv_norm),
            layer=layer,
            cached_tokens=shape.cached_tokens,
            width=spec.compressed_width,
        ...
        if shape.phase == "prefill":
            kv_b = self.matrix(
                ...
                kv_norm,
                spec.kv_rank,
                spec.heads * (spec.score_nope_dim + spec.value_dim),
                layer=layer,
            )
            ...
            absorbed = self.matrix(
                "mla.query_absorption",
                q_b,
                spec.score_nope_dim,
                spec.kv_rank,
                layer=layer,
        ...
        score = self.region(
            "mla.query_key_product",
            score_parents,
            layer=layer,
            heads=spec.heads,
            width=score_width,
            pairs=shape.causal_pairs,
            cached_tokens=shape.cached_tokens,
            variant=variant,
        ...
        value = self.region(
            ...
            pairs=shape.causal_pairs,
            variant=variant,
            flops=2 * shape.causal_pairs * spec.heads * value_width,
        )
        if shape.phase == "decode":
            value = self.matrix(
                "mla.value_reexpansion",
                value,
                spec.kv_rank,
                spec.value_dim,
                layer=layer,
        ...
        gate = self.matrix(
            "mla.output_gate_projection", parent, h, spec.heads * spec.value_dim, layer=layer
        )
        gated = self.region(
            "mla.sigmoid_multiply",
            (value, gate),
            layer=layer,
            heads=spec.heads,
            width=spec.value_dim,
        )
        return self.matrix(
            "mla.output_projection", gated, spec.heads * spec.value_dim, h, layer=layer
        )
    ...
    def dense(self, parent: str, layer: int) -> str:
        ...
        active = self.region(
            ...
            layer=layer,
            width=width,
            beta=self.spec.moe.situ_beta,
            linear_beta=self.spec.moe.situ_linear_beta,
        )
    ...
    def moe(self, parent: str, layer: int) -> str:
        ...
        e, width, visits = (
            ...
            self.shape.new_tokens * spec.selected_experts,
        )
        router = self.matrix("moe.router_projection", parent, h, spec.experts, layer=layer)
        select = self.region(
            "moe.expert_selection",
            (router,),
            layer=layer,
            resident_experts=spec.experts,
            selected_experts=spec.selected_experts,
            correction_bias=True,
            renormalize=spec.renormalize,
            activation=spec.router_activation,
            topk_method=spec.topk_method,
        )
        latent = self.matrix("moe.latent_down_projection", parent, h, e, layer=layer)
        dispatch = self.region(
            "moe.dispatch",
            (select, latent),
            layer=layer,
            latent_width=e,
            selected_experts=spec.selected_experts,
            expert_token_visits=visits,
            routing_scope="logical-selections-without-rank-placement",
        )
        gate = self.matrix(
            "moe.expert_gate_projection", dispatch, e, width, layer=layer, tokens=visits
        )
        ...
        active = self.region(
            ...
            width=width,
            beta=spec.situ_beta,
            linear_beta=spec.situ_linear_beta,
        )
        down = self.matrix(
            "moe.expert_down_projection", active, width, e, layer=layer, tokens=visits
        )
        combined = self.region(
            "moe.weighted_combine",
            (down, select),
            layer=layer,
            selected_experts=spec.selected_experts,
            width=e,
        ...
        shared_gate = self.matrix(
            "moe.shared_gate_projection", parent, h, shared_width, layer=layer
        )
        shared_up = self.matrix("moe.shared_up_projection", parent, h, shared_width, layer=layer)
        shared_active = self.region(
            "moe.shared_situ",
        ...
        shared = self.matrix(
            "moe.shared_down_projection", shared_active, shared_width, h, layer=layer
        )
    ...
    def build(self) -> ExecutionGraph:
        ...
        parent = self.region(
            "embedding", (), width=spec.hidden_size, vocabulary=spec.vocab_size, flops=0
        )
        for layer in spec.layers:
            before = layer.residual_bank_before
            sources = 0 if layer.index == 0 else before + 1
            mixed = self.region(
                f"residual.attention_sources_{sources}",
                (parent,),
                layer=layer.index,
                width=spec.hidden_size,
                bank_before=before,
                source_vectors=sources,
                snapshot=layer.writes_residual_snapshot,
                rms_epsilon=spec.rms_norm_epsilon,
            )
            attention = (
                self.kda(mixed, layer.index)
                if layer.attention == "kda"
                else self.mla(mixed, layer.index)
            )
            bank = before + int(layer.writes_residual_snapshot)
            post = self.region(
                f"residual.mlp_sources_{bank + 1}",
                (attention, mixed),
                layer=layer.index,
                width=spec.hidden_size,
                bank_after=bank,
                source_vectors=bank + 1,
                reset_prefix=layer.writes_residual_snapshot,
                rms_epsilon=spec.rms_norm_epsilon,
            )
            feed_forward = (
                self.dense(post, layer.index)
                if layer.feed_forward == "dense"
                else self.moe(post, layer.index)
            )
            parent = self.region(
                "residual.ffn_delta_prefix_add",
                (feed_forward, post),
                layer=layer.index,
                width=spec.hidden_size,
            )
        ...
        mixed = self.region(
            "final_residual_mixture", (parent,), width=spec.hidden_size, source_vectors=bank + 1
        )
        ...
        head = self.matrix(
            "lm_head", norm, spec.hidden_size, spec.vocab_size, tokens=shape.sampled_tokens
        )
        graph = ExecutionGraph(
            self.execution_id,
            self.record.step_index,
            self.record.virtual_time_ps,
...
class KimiK3Lowerer:
    ...
    def lower(
        self, record: StepRecord, observations: ExecutionObservations | None = None
    ) -> ExecutionGraph:
        if observations is not None:
            raise ValueError(
                "native physical observations require their own total implementation binding"
            )
        return _GraphBuilder(record, self.config).build()


def bind_synthetic_kimi_k3_graph(
    graph: ExecutionGraph, service_ps_per_token: int
) -> ExecutionGraph:
    ...
    embeddings = [
        ...
        if isinstance(op.work, ComputeWork) and op.work.kernel == "kimi_k3.embedding"
    ]
    if len(embeddings) != 1:
    ...
    for operation in graph.operations:
        ...
        if not isinstance(work, ComputeWork) or work.scope != "logical-operator":
            raise ValueError(
                "synthetic K3 binding requires every region to be unbound logical work"
            )
        config = dict(work.config)
        config.update(
            binding="uniform-logical-service-v1",
            source_graph_sha256=source,
            synthetic_memory_arbitration="zero",
            service_ps_per_token=service_ps_per_token,
        )
        operations.append(
            replace(
                operation,
                work=replace(
                    ...
                    flops=0,
                    hbm_bytes=0,
                    nominal_duration_ps=tokens * service_ps_per_token,
                    scope="synthetic-operator",
                ),
            )
        )
    bound = replace(
        graph,
        execution_id=f"{graph.execution_id}:synthetic:{service_ps_per_token}",
