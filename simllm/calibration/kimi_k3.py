...
def validate_kimi_k3_native_contract(
    ...
    if text.model_type != "kimi_linear" or tuple(text.architectures or ()) != (
    ...
    if (
        quantization.get("quant_method") != quant_method
    ...
    if any(
        weights.get(key) != value or type(weights.get(key)) is not type(value)
...
def validate_kimi_k3_suite(suite: Mapping[str, Any]) -> None:
    ...
    scope = {
        "mode": "text-only",
        "included_phases": ["prefill", "decode"],
        "optional_components": [],
        "excluded_components": [
            "vision",
            "projector",
...
def project_kimi_k3_graphs(
    records: tuple[StepRecord, ...], spec: KimiK3Spec, framework: str
    ...
    *,
    ...
    for family in families:
        ...
        for by_family in grouped:
            ...
            vectors.append(
                (len(operations), *(value for _, value in operator_axes(operations[0])))
                if operations
                else (0,) * len(names)
            )
            ...
            ShapeSchema(
                ...
                tuple(
                    ShapeAxis(
                        name,
                        "logical-visits"
                        if name == "logical_visits"
                        else "tokens"
                        if "tokens" in name
                        else "pairs"
                        if name == "pairs"
                        else "elements",
                        min(vector[i] for vector in vectors),
                        max(vector[i] for vector in vectors),
                    )
    ...
    for record, graph, by_family, cell in zip(records, graphs, grouped, cells, strict=True):
        ...
        for family, schema in zip(families, schemas, strict=True):
            ...
            projections.append(
                KernelProjection(
                    family,
                    ShapeVector(schema.shape_schema_id, values),
                    len(operations),
                    aggregate,
        ...
        if sum(projection.logical_launch_count for projection in projections) != len(
            graph.operations
        ):
    ...
    inventory = ModelKernelInventory(
        ...
        framework=framework,
        model=model,
        shape_schemas=tuple(schemas),
        kernel_families=tuple(definitions),
        cases=tuple(cases),
            ...
            join_tasks=tuple(
                sorted(
                    (
                        "COMP-6",
                        "COMP-59",
                        "VLLM-12" if framework.framework_id == "vllm" else "SGL-10",
                    )
                )
            ),
        ),
