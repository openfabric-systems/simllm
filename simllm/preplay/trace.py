...
def _sampling_from_json(value: object, path: str) -> SamplingConfig:
    ...
    seed = None if seed_value is None else _integer(seed_value, f"{path}.seed", nonnegative=True)
...
def trace_provenance_from_json(value: object, path: str = "provenance") -> TraceProvenance:
    ...
    provenance = TraceProvenance(
        ...
        top_k=_integer(payload["top_k"], f"{path}.top_k", minimum=1),
        expert_count=_integer(
            payload["expert_count"],
            f"{path}.expert_count",
            minimum=1,
        ),
        moe_layer_indices=_int_tuple(
...
def _routing_from_json(value: object, path: str) -> LayerRouting:
    ...
    weights = tuple(
        _number(weight, f"{path}.gate_weights[{index}]", nonnegative=True)
        for index, weight in enumerate(_array(payload["gate_weights"], f"{path}.gate_weights"))
    )
    return LayerRouting(
        ...
        expert_ids=_int_tuple(payload["expert_ids"], f"{path}.expert_ids"),
...
def _token_row(request_id: str, token: TokenTrace) -> dict[str, Any]:
    return {
        "schema": PREPLAY_TRACE_SCHEMA,
        "row_type": _TOKEN,
...
def _request_header_from_row(payload: Mapping[str, Any], path: str) -> _RequestHeader:
    ...
    return _RequestHeader(
        ...
        input_token_ids=_int_tuple(payload["input_token_ids"], f"{path}.input_token_ids"),
...
def _token_from_row(
    ...
    *,
    ...
    _validate_row_envelope(payload, path, _TOKEN)
    ...
    actual_index = _integer(payload["token_index"], f"{path}.token_index", nonnegative=True)
    if actual_index != token_index:
        _fail(f"{path}.token_index", f"expected contiguous index {token_index}")
    return TokenTrace(
        token_index=actual_index,
        token_id=_integer(payload["token_id"], f"{path}.token_id", nonnegative=True),
        routing=tuple(
...
class PreplayTraceWriter:
    """Append validated requests and flush each one before accepting the next."""

    def __init__(self, path: str | Path, provenance: TraceProvenance):
        validate_trace_provenance(provenance)
        self.path = Path(path)
    ...
    def append(self, request: RequestTrace) -> None:
        ...
        for token in request.tokens:
            self._write_row(_token_row(request.request_id, token))
        self._handle.flush()
        self._request_ids.add(request.request_id)
        self._request_count += 1
        self._token_count += len(request.tokens)
...
class PreplayTraceReader:
    ...
    def _finish(self, payload: Mapping[str, Any], path: str) -> None:
        _fields(
            payload,
            path,
            required={"schema", "row_type", "request_count", "token_count"},
        )
        ...
        if request_count != self._request_count:
            _fail(
                f"{path}.request_count",
                f"declares {request_count}, observed {self._request_count}",
            )
        if token_count != self._token_count:
            _fail(
                f"{path}.token_count",
                f"declares {token_count}, observed {self._token_count}",
            )
    ...
    def __next__(self) -> RequestTrace:
        ...
        for token_index in range(header.output_token_count):
            token_payload, token_path = self._read_required_row(
                f"token row {token_index} for request {header.request_id!r}"
            )
            tokens.append(
                _token_from_row(
                    token_payload,
...
def write_preplay_trace(
    ...
    """Stream requests to ``path`` without materializing the iterable."""

    with PreplayTraceWriter(path, provenance) as writer:
...
def read_preplay_trace(path: str | Path) -> PreplayTrace:
    """Load and validate a complete trace."""

    with PreplayTraceReader(path) as reader:
        trace = PreplayTrace(provenance=reader.provenance, requests=tuple(reader))
